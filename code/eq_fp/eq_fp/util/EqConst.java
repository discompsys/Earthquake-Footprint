...
public class EqConst {
     ...
     */
    public static String toEventKey(int numGaps, double avGap, double numEvents) {
     ...
     * @param avGap average gap size in time units.
     * @param numEvents number of events in related sequences.
     * @param avCoh average day cohesion.
     * @return descriptive key.
     */
    public static String toEventKey(int numGaps, double avGap, double numEvents,
                                    double avCoh) {
        ...
        eventKey = (String.valueOf(numGaps) + EqConst.COMPSEP);
        eventKey += (dc.format(avGap) + EqConst.COMPSEP);
        eventKey += (String.valueOf(numEvents) + EqConst.COMPSEP);
        eventKey += (dc.format(avCoh));
    ...
    public static Object[] fromEventKey(String eventKey) {
        ...
        eventParts = new Object[tokens.size()];
        eventParts[0] = Integer.parseInt(tokens.get(0));
        eventParts[1] = Double.parseDouble(tokens.get(1));
        if (tokens.size() > 2) {
            eventParts[2] = Double.parseDouble(tokens.get(2));
        }
        else {
            eventParts[2] = null;
        }
        if (tokens.size() > 3) {
            eventParts[3] = Double.parseDouble(tokens.get(3));
        }
    ...
    /**
     * Calculate the variance for the list of differences.
     * @param tsDiff list of day gaps.
     * @param eventAv average number of relevant events each day.
     * @return
     */
    public static double cohesion(ArrayList<Double> tsDiff, double eventAv) {
        double av, var;

        av = 0;
        for (double number : tsDiff) {
            av += number;
        }
        av /= tsDiff.size();
        ...
        var = 0;
        for (double number : tsDiff) {
            var += Math.pow((number - av), 2);
        }
        var = (var / tsDiff.size()-1);
